    ...
    <Page>
      <PageHeader>
        <div>
          ...
          <p className="text-xs text-muted-foreground">Live overview of parking operations.</p>
        </div>
        <PageActions>
          <Button variant="outline" size="sm" onClick={refresh} disabled={anyRefreshing} aria-label={anyRefreshing ? 'Refreshing data…' : 'Refresh dashboard data'}>
            <RefreshCw className={`h-4 w-4 mr-1.5 ${anyRefreshing ? 'animate-spin' : ''}`} aria-hidden="true" />
            {anyRefreshing ? 'Refreshing…' : 'Refresh'}
          </Button>
          <Button asChild size="sm"><Link to="/ocr"><ScanLine className="h-4 w-4 mr-1.5" aria-hidden="true" />Scan Permit</Link></Button>
        </PageActions>
      </PageHeader>
      <PageBody className="space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" role="list" aria-label="Key performance indicators">
          <KpiCard label="Total Spots" value={formatNumber(kpis.totalSpots)} icon={<MapPin className="h-4 w-4" aria-hidden="true" />} hint={<>{kpis.availableSpots} available · {kpis.occupiedSpots} occupied</>} accent="info" />
          <KpiCard label="Active Sessions" value={formatNumber(kpis.activeSessions)} icon={<Activity className="h-4 w-4" aria-hidden="true" />} hint={`${kpis.completedToday} completed today`} accent="success" />
          <KpiCard label="Vehicles" value={`${kpis.registeredVehicles} / ${kpis.totalVehicles}`} icon={<Car className="h-4 w-4" aria-hidden="true" />} hint="Registered in registry" accent="default" />
          <KpiCard label="Active Rules" value={formatNumber(kpis.activeRules)} icon={<ShieldCheck className="h-4 w-4" aria-hidden="true" />} hint="Enforced in real-time" accent="info" />
        </div>
        {kpis.violations > 0 && (
          <Card className="p-4 flex items-center gap-3" role="alert" aria-label={`${kpis.violations} parking violation${kpis.violations === 1 ? '' : 's'} detected`}>
            <span className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300" aria-hidden="true"><AlertTriangle className="h-4 w-4" /></span>
            <div className="flex-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wider">Violations</p>
              <p className="text-xl font-semibold tabular-nums">{kpis.violations}</p>
            </div>
          </Card>
        )}
        ...
        {isLoading && <p className="text-center text-xs text-muted-foreground py-4" role="status" aria-live="polite">Loading latest data…</p>}
      </PageBody>
    </Page>
  )
}
...
function RecentSessionsCard({ sessions }: { sessions: ParkingSession[] }) {
  ...
  return (
    <Card className="p-5">
      ...
      <ul className="space-y-2" aria-label="Recent parking sessions">
        {sessions.map(s => {
          const statusLabel = s.status === 'active' ? 'Active' : s.status === 'violation' ? 'Violation' : 'Completed'
          return (
            <li key={s.id} className="flex items-center gap-3 rounded-md border border-border p-2.5 hover:bg-muted/30 transition-colors">
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-muted text-muted-foreground shrink-0" aria-hidden="true"><Car className="h-4 w-4" /></span>
              ...
              <Badge
                variant={s.status === 'active' ? 'default' : s.status === 'violation' ? 'destructive' : 'secondary'}
                className={s.status === 'active' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800' : ''}
