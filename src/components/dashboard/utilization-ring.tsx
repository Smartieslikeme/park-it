...
export function UtilizationRing({ pct, total, occupied, available }: Props) {
  ...
  const statusText = safePct >= 90 ? 'High occupancy' : safePct >= 70 ? 'Moderate occupancy' : 'Low occupancy'
  ...
  return (
    <Card className="p-5 flex flex-col">
      ...
      <div className="flex items-center gap-4 flex-1">
        <div className="relative h-32 w-32 shrink-0" role="img" aria-label={`Parking utilization: ${safePct}% occupied (${statusText}). ${occupied} spots occupied, ${available} available out of ${total} total.`}>
        ...
        <div className="space-y-2 flex-1 min-w-0">
          <div className="flex items-center justify-between text-xs">
            <span className="inline-flex items-center gap-1.5 text-muted-foreground">
              <span className="h-2 w-2 rounded-full bg-rose-500" aria-hidden="true" />Occupied
            </span>
