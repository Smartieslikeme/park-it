  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between mb-4">
        <div><h3 className="text-sm font-semibold">Weekly Occupancy</h3><p className="text-xs text-muted-foreground">{hasData ? 'Sessions from the last 7 days' : 'Awaiting first sessions'}</p></div>
      ...
      <div className="h-[220px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            ...
            <Bar dataKey="available" stackId="a" fill={MUTED} radius={[0, 0, 0, 0]} />
            <Bar dataKey="occupied" stackId="a" fill={PRIMARY} radius={[3, 3, 0, 0]}>{data.map((_, i) => <Cell key={i} fill={PRIMARY} />)}</Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
    </Card>
